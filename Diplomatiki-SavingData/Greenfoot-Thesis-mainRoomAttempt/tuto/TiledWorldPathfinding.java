...
public class TiledWorldPathfinding {
    ...
    public Stack<Point> findShortestFeasiblePath(Point start,
    Point end) {
    ...
    Queue<Tile> closedList, Point end) {
        ...
        while( !openList.isEmpty() && !done ) {
            t = openList.remove();
            done = done || processNeighbor(t, t.getUp(),
                openList, end);
            done = done || processNeighbor(t, t.getDown(),
                openList, end);
            done = done || processNeighbor(t, t.getLeft(),
                openList, end);
            done = done || processNeighbor(t, t.getRight(),
                openList, end);
            t.setDone();
            closedList.add(t);
        }
    ...
    private boolean processNeighbor( Tile parent, Tile node,
    Queue<Tile> openList, Point end) {
